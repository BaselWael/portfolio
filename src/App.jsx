...
import {
  ...
  Languages, Download, Menu, X, Sparkles, CheckCircle2,
  Code2, LayoutDashboard, ShoppingCart, Quote, ChevronLeft, ChevronRight
} from "lucide-react";
...
const profile = {
  ...
  taglineAr: "Project Manager & Full-Stack — ببني أنظمة ويب وموبايل سريعة مع تجربة مستخدم مميزة.",
  taglineEn: "Project Manager & Full-Stack — I ship fast web/mobile systems with great UX.",
...
/* Skills */
const skills = {
  frontend: ["React", "Next.js", "TailwindCSS", "Framer Motion", "Vite", "Zustand"],
  backend: ["Laravel", "Node.js", "Express", "REST", "WebSockets", "Prisma"],
  devops: ["Nginx", "Docker", "CI/CD", "Linux", "Cloudflare"],
  tools: ["Git", "MySQL", "Postman", "Stripe", "Redis"],
};
    ...
    lang === "ar"
      ? {
          ...
          about: {
            header: "من أنا",
            paragraphs: [
              "مدير مشاريع وتقني أقود فرق تطوير ويب وموبايل من الفكرة إلى الإطلاق مع تركيز كبير على تجربة المستخدم والأداء والحركة السلسة.",
              "أحوّل الرؤية إلى Roadmap واضحة، أقسّم العمل إلى Sprints قصيرة، أتابع التقدّم بالمؤشرات، وأدير المخاطر والتواصل مع أصحاب المصلحة.",
              "أهتم بكتابة وثائق المتطلبات (SRS) وتحديد النطاق، وضع KPIs، ثم الإشراف على الجودة والاختبارات والنشر على خوادم آمنة.",
              "خبرتي تشمل تنظيم التكاملات (مدفوعات، إشعارات، CDN)، إعداد CI/CD، ومراقبة الأعطال وتحليلات الاستخدام بعد الإطلاق لضمان استقرار التجربة."
            ],
            bullets: ["إدارة مشاريع", "Roadmaps & Sprints", "توثيق SRS", "أنيميشن وتجربة مستخدم", "أداء عالي", "نشر آمن وCI/CD"],
            facts: [
              {k:"مشاريع مُدارة",v:"25+"},
              {k:"إطلاقات إنتاجية",v:"10+"},
              {k:"فرق قُدتها",v:"3+"},
              {k:"دول مستهدفة",v:"5+"}
            ],
          },
      ...
      : {
          ...
          about: {
            header: "About Me",
            paragraphs: [
              "Project Manager & technologist leading web/mobile teams from idea to launch, with a focus on UX, performance and smooth motion.",
              "I turn vision into a clear roadmap, break work into short sprints, track progress with metrics, and manage risks & stakeholder comms.",
              "I draft SRS documents, scope work and KPIs, then oversee quality, testing and secure deployments.",
              "Experience includes integrations (payments, notifications, CDN), setting up CI/CD, and post-launch monitoring & analytics for stable UX."
            ],
            bullets: ["Project Management", "Roadmaps & Sprints", "SRS Documentation", "Motion & UX", "High Performance", "Secure Deployments & CI/CD"],
            facts: [
              {k:"Managed Projects",v:"25+"},
    ...
    <div>
      ...
      <div dir={dir} className="relative min-h-screen bg-[#f6f7fb] text-slate-900 overflow-x-clip">
        ...
        <main id="home" className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          ...
          {/* Services */}
          <Section id="services">
            <motion.h2 {...fadeUp(0)} className="text-2xl md:text-3xl font-bold mb-6">{t.services.header}</motion.h2>
            <div className="grid gap-6 md:grid-cols-3">
              {[
                { icon: <Code2 className="h-5 w-5" />, titleAr: "مواقع سريعة 🚀", titleEn: "Blazing Websites", pointsAr: ["Landing page مبهرة", "SEO أساسي", "Responsive بالكامل"], pointsEn: ["Stunning landing page", "Basic SEO", "Fully responsive"], price: "Starting at $199" },
                { icon: <LayoutDashboard className="h-5 w-5" />, titleAr: "لوحات تحكم تفاعلية", titleEn: "Interactive Dashboards", pointsAr: ["Charts + Filters", "Auth & Roles", "أداء ممتاز"], pointsEn: ["Charts + Filters", "Auth & Roles", "Great performance"], price: "Starting at $499" },
                { icon: <ShoppingCart className="h-5 w-5" />, titleAr: "متاجر إلكترونية", titleEn: "E-Commerce Stores", pointsAr: ["بوابات دفع", "منتجات/عربات", "حملات وتسويق"], pointsEn: ["Payment gateways", "Products/Carts", "Campaigns & marketing"], price: "Starting at $699" },
              ].map((s,i)=>(
                <motion.div key={i} {...fadeUp(0.05*i)} className="group">
                  <div className="rounded-2xl p-[1px] bg-gradient-to-br from-indigo-500/40 via-fuchsia-500/40 to-cyan-500/40 group-hover:via-indigo-500/70 transition">
                    <Card interactive={true} className="h-full">
                      <div className="p-6">
                        <motion.div whileHover={{ rotate: 8 }} className="h-10 w-10 rounded-xl bg-indigo-50 text-indigo-600 grid place-content-center mb-4">{s.icon}</motion.div>
                        <h3 className="text-lg font-semibold mb-2">{lang==="ar"?s.titleAr:s.titleEn}</h3>
                        <ul className="space-y-1.5 text-slate-700 mb-4">
                          {(lang==="ar"?s.pointsAr:s.pointsEn).map((p,idx)=>(
                            <li key={idx} className="inline-flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-indigo-500"/>{p}</li>
                          ))}
                        </ul>
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-medium text-slate-600">{s.price}</span>
                          <Button asChild href="#contact" size="sm"><span>{lang==="ar"?"اطلب الخدمة":"Get it"}</span></Button>
                        </div>
                      </div>
                    </Card>
                  </div>
          ...
          {/* Skills */}
          <Section id="skills">
            <motion.h2 {...fadeUp(0)} className="text-2xl md:text-3xl font-bold mb-6">{t.skills.header}</motion.h2>
            <Tabs defaultValue="frontend">
              ...
              {Object.entries(skills).map(([key, list])=>(
                <TabsContent key={key} value={key}>
                  <Card><CardContent className="pt-6"><div className="flex flex-wrap gap-2">{list.map(s=><Badge key={s}>{s}</Badge>)}</div></CardContent></Card>
                </TabsContent>
              ))}
            </Tabs>
          </Section>
